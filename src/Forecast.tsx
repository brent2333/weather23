    ...
    <div>
      <div className="flex">
        ...
        <button
          ...
          className="inline text-white text-xs bg-slate-800 p-1.5 left-34 top-1.5 rounded-lg"
        >
          {" "}
          &#176;F / &#176;C
        </button>
      ...
      <div className="lg:flex">
        {!currentForecast ? (
          ...
          currentForecast?.forecastday.map((day, index: number) => {
            return (
              <div
                className="flex-1 shadow-md rounded m-4  p-8 bg-gray-300"
                key={`forecast${index}`}
              >
                <h3 className="font-bold text-xl text-black tracking-tight mb-2">
                  {
                    new Date(day.date.replace("-", "/"))
                      .toString()
                      .split(" ")[0]
                  }
                </h3>
                  ...
                  <div className="basis-full">
                    {unit ? (
                      <Fragment>
                        <div>
                          Low:{" "}
                          <span className="font-bold">{day.day.mintemp_f}</span>{" "}
                          &#176;F
                        </div>
                        <div>
                          High:{" "}
                          <span className="font-bold">{day.day.maxtemp_f}</span>{" "}
                          &#176;F
                        </div>
                      </Fragment>
                    ) : (
                      <Fragment>
                        <div>
                          Low:{" "}
                          <span className="font-bold">{day.day.mintemp_c}</span>{" "}
                          &#176;C
                        </div>
                        <div>
                          High:{" "}
